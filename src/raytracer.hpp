...
class RayTracer {
public:
    /// Tracks the current medium the ray is traversing.
    ///
...
public:
    /// Clamps the radiance values to avoid fireflies/NaNs.
    /// @param c Input color.
    /// @param max_component Maximum allowed component value.
...
public:
    /// Renders a single pixel using standard pinhole camera model.
    ...
    /// Renders a single pixel showing normal colors (for debugging).
    ///
    ...
    /// Renders a single pixel with Depth of Field (DoF).
    /// @param cam The camera.
    /// @param x Pixel x-coordinate.
    /// @param y Pixel y-coordinate.
    /// @param width Image width.
...
private:
    ...
    /// Intersects the ray with the scene geometry.
    RayTriangleIntersection hit(const glm::vec3& ro, const glm::vec3& rd) const noexcept;

    /// Computes visibility between two points (shadow ray).
    /// @return Transmission color (e.g., white if visible, black if blocked,
    ///         or filtered color if through transparent object).
