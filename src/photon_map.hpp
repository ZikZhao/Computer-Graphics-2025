...
/// Represents a single photon used for caustic estimation.
struct Photon {
...
/// Hash functor for integer 3D grid coordinates.
struct GridCellHash {
...
/// Photon mapping accelerator for estimating caustic radiance.
///
...
public:
    /// Checks if material is transparent (refractive).
...
public:
    ...
    /// Retrieves photons within a radius of a point.
    ...
    /// Estimates caustic radiance at a surface point.
...
private:
    /// Builds the photon map in a background thread.
    ///
    ...
    /// Emits a batch of photons from an area light.
    ...
    /// Normalizes all stored photon powers based on total emitted count.
    ...
    /// Traces a single photon through the scene.
