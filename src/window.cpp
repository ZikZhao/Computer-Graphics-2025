        ...
        if (IsPressedMode(binding.trigger)) {
            ...
            if (binding.trigger == Trigger::ANY_DOWN || binding.trigger == Trigger::ANY_PRESSED || binding.trigger == Trigger::ANY_PRESSED_NO_MODIFIER) {
                ...
                for (auto k : binding.keys) {
                    if (keys_this_frame_[k]) { any_down = true; break; }
                }
            } else if (binding.trigger == Trigger::ALL_DOWN || binding.trigger == Trigger::ALL_PRESSED) {
                any_down = true;
                for (auto k : binding.keys) {
                    if (!keys_this_frame_[k]) { any_down = false; break; }
                }
            }
...
bool Window::has_modifier_keys() const noexcept {
    return keys_this_frame_[SDL_SCANCODE_LCTRL] ||
           ...
           keys_this_frame_[SDL_SCANCODE_RSHIFT] ||
           keys_this_frame_[SDL_SCANCODE_LALT] ||
...
void Window::clear_pixels() noexcept {
    std::fill(pixel_buffer_.begin(), pixel_buffer_.end(), 0);
}
...
void Window::save_bmp(const std::string& filename) const {
    auto surface = SDL_CreateRGBSurfaceFrom(
        (void*)pixel_buffer_.data(), width_, height_, 32,
