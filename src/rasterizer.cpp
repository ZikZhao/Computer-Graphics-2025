...
Rasterizer::Rasterizer(Window& window) : window_(window), z_buffer_(window.get_width() * window.get_height(), 0.0f) {}

void Rasterizer::clear() noexcept { z_buffer_.assign(window_.get_width() * window_.get_height(), 0.0f); }
...
FloatType Rasterizer::get_depth(int x, int y) const noexcept {
    if (x < 0 || static_cast<size_t>(x) >= window_.get_width() || y < 0 || static_cast<size_t>(y) >= window_.get_height())
        return 0.0f;
    return z_buffer_[static_cast<size_t>(y) * window_.get_width() + static_cast<size_t>(x)];
}
...
void Rasterizer::wireframe_render(
    ...
    for (size_t i = 0; i < screen_verts.size(); i++) {
        ...
        if (std::abs(to.x - from.x) >= std::abs(to.y - from.y)) {
            ...
            std::size_t to_x = std::min<std::size_t>(static_cast<std::size_t>(std::max(from.x, to.x)), width - 1);
            for (std::size_t x = from_x; x <= to_x; x++) {
                std::int64_t y =
                    static_cast<std::int64_t>(std::round(from.y + (to.y - from.y) * (x - from.x) / (to.x - from.x)));
                if (x >= static_cast<std::size_t>(width) || y < 0 || y >= static_cast<std::int64_t>(height)) continue;
                FloatType progress = (to.x == from.x)
                                         ? 0.0f
                                         : static_cast<FloatType>(x - from.x) / static_cast<FloatType>(to.x - from.x);
            ...
            for (std::size_t y = from_y; y <= to_y; y++) {
                std::int64_t x =
                    static_cast<std::int64_t>(std::round(from.x + (to.x - from.x) * (y - from.y) / (to.y - from.y)));
                if (x < 0 || x >= static_cast<std::int64_t>(width) || y >= static_cast<std::size_t>(height)) continue;
                FloatType progress = (to.y == from.y)
                                         ? 0.0f
                                         : static_cast<FloatType>(y - from.y) / static_cast<FloatType>(to.y - from.y);
                FloatType inv_z = ComputeInvZndc(progress, std::array<FloatType, 2>{from.z_ndc, to.z_ndc});
                FloatType& depth = z_buffer_[y * width + x];
                if (inv_z > depth) {
                    depth = inv_z;
...
void Rasterizer::rasterized_render(
    ...
    for (size_t i = 1; i + 1 < clipped.size(); i++) {
        ...
        std::int64_t mid_y =
            std::min<std::int64_t>(static_cast<std::int64_t>(std::ceil(v1.y)), static_cast<std::int64_t>(height - 1));
        std::int64_t to_y =
            std::min<std::int64_t>(static_cast<std::int64_t>(std::ceil(v2.y)), static_cast<std::int64_t>(height - 1));
...
constexpr FloatType Rasterizer::ComputeIntersectionT(
    const glm::vec4& v0, const glm::vec4& v1, ClipPlane plane) noexcept {
    FloatType d0, d1;
    switch (plane) {
    case ClipPlane::LEFT:
        d0 = v0.x + v0.w;
